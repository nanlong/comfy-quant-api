...
pub async fn create(db: &PgPool, data: &StrategySpotStats) -> Result<StrategySpotStats> {
    let strategy_position = sqlx::query_as!(
        StrategySpotStats,
        r#"
        INSERT INTO strategy_spot_stats (workflow_id, node_id, node_name, exchange, market, symbol, base_asset, quote_asset, initial_base_balance, initial_quote_balance, maker_commission_rate, taker_commission_rate, base_asset_balance, quote_asset_balance, avg_price, total_trades, buy_trades, sell_trades, total_base_volume, total_quote_volume, total_base_commission, total_quote_commission, realized_pnl, win_trades, created_at, updated_at)
...
mod tests {
    ...
    async fn test_strategy_spot_stats_create(db: PgPool) -> Result<()> {
        ...
        assert_eq!(strategy_spot_stats.market, "spot");
